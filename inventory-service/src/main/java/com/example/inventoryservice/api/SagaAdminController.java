...
import com.example.inventoryservice.domain.SagaControlService;
import com.example.inventoryservice.domain.SagaLogRecord;
...
public class SagaAdminController {
    ...
    public SagaAdminController(InventoryService inventoryService, SagaControlService sagaControlService) {
    ...
    public ResponseEntity<List<SagaLogRecord>> logs() {
        return ResponseEntity.ok(inventoryService.findSagaLogs());
    }

    @PostMapping("/logs/{logId}/replay")
    public ResponseEntity<Void> replay(@PathVariable Long logId) {
        inventoryService.replaySagaLog(logId);

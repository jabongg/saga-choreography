...
import com.example.notificationservice.domain.SagaControlService;
import com.example.notificationservice.domain.SagaLogRecord;
...
public class SagaAdminController {
    ...
    public SagaAdminController(NotificationService notificationService, SagaControlService sagaControlService) {
    ...
    public ResponseEntity<List<SagaLogRecord>> logs() {
        return ResponseEntity.ok(notificationService.findSagaLogs());
    }

    @PostMapping("/logs/{logId}/replay")
    public ResponseEntity<Void> replay(@PathVariable Long logId) {
        notificationService.replaySagaLog(logId);

...
import com.example.paymentservice.domain.SagaControlService;
import com.example.paymentservice.domain.SagaLogRecord;
...
public class SagaAdminController {
    ...
    public SagaAdminController(PaymentService paymentService, SagaControlService sagaControlService) {
    ...
    public ResponseEntity<List<SagaLogRecord>> logs() {
        return ResponseEntity.ok(paymentService.findSagaLogs());
    }

    @PostMapping("/logs/{logId}/replay")
    public ResponseEntity<Void> replay(@PathVariable Long logId) {
        paymentService.replaySagaLog(logId);
